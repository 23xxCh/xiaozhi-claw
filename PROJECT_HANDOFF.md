...
> 产品化最新批次：[手机配网、认领恢复及固件候选记录](docs/reports/2026-09-07-productization-onboarding.md)。统一入口 `/setup` 已在本地运行；有效码复用、逐台进度和设备服务地址保护已实现。用户已通过网页完成本地认领；补齐 NOCAM 待机预连接和绑定轮询后，最新应用已烧录、完整哈希核对通过，真实网关会话为 online。Wi-Fi、身份和表情资源均保留。真实收音、扬声器、表情、配置回执及换网恢复继续验收，不能把在线当作整机通过。
