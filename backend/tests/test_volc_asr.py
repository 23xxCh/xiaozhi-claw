...
def response(data, flags=1):
    ...
    return bytes((0x11, 0x90 | flags, 0x11, 0))+struct.pack('>iI', -1 if flags == 3 else 1, len(payload))+payload
...
@pytest.mark.parametrize('raw', [b'', b'\x11\x91\x11\0', response({})[:-1], response({})+b'x',
