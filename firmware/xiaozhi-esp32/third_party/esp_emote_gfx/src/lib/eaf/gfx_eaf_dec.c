...
#define EAF_HUFFMAN_MAX_SYMBOLS           (256)
#define EAF_HUFFMAN_MAX_NODES             (EAF_HUFFMAN_MAX_SYMBOLS * 2 - 1)
...
static esp_err_t huffman_decode_data(const uint8_t *in_data, size_t in_size,
                                     ...
                                     uint8_t *out_data, size_t *out_size)
{
    if (!in_data || !dict_data || !out_data || !out_size || in_size == 0 || dict_len < 2) {
        return ESP_ERR_INVALID_ARG;
    }

    const size_t out_capacity = *out_size;
    *out_size = 0;

    uint8_t padding_bits = dict_data[0];
    if (padding_bits > 7 || padding_bits > in_size * 8) {
        GFX_LOGE(TAG, "Invalid Huffman padding: %u", padding_bits);
        return ESP_FAIL;
    }

    eaf_dec_huffman_node_t *nodes = heap_caps_calloc(
            EAF_HUFFMAN_MAX_NODES, sizeof(eaf_dec_huffman_node_t),
            MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (!nodes) {
        GFX_LOGE(TAG, "No internal memory for Huffman tree");
    ...
    size_t dict_pos = 1;
    eaf_dec_huffman_node_t *root = &nodes[0];
    eaf_dec_huffman_node_t *current_node = NULL;

    while (dict_pos < dict_len) {
        if (dict_len - dict_pos < 2) {
            GFX_LOGE(TAG, "Truncated Huffman dictionary header");
            goto cleanup;
        }
        ...
        if (code_len == 0 || code_len > 64) {
            GFX_LOGE(TAG, "Invalid Huffman code length: %u", code_len);
            goto cleanup;
        }

        size_t code_byte_len = (code_len + 7) / 8;
        if (code_byte_len > dict_len - dict_pos) {
            GFX_LOGE(TAG, "Truncated Huffman dictionary code");
            goto cleanup;
        }
        const uint8_t *code_bytes = dict_data + dict_pos;
        dict_pos += code_byte_len;

        current_node = root;
        for (int bit_pos = code_len - 1; bit_pos >= 0; --bit_pos) {
            if (current_node->is_leaf) {
                GFX_LOGE(TAG, "Invalid Huffman prefix collision");
                goto cleanup;
            }

            size_t code_byte_index = code_byte_len - 1 - ((size_t)bit_pos / 8);
            int bit_val = (code_bytes[code_byte_index] >> (bit_pos % 8)) & 1;
            eaf_dec_huffman_node_t **child = bit_val == 0 ? &current_node->left : &current_node->right;
            if (!*child) {
                if (nodes_used >= EAF_HUFFMAN_MAX_NODES) {
                    GFX_LOGE(TAG, "Huffman tree exceeds node limit");
                    goto cleanup;
                }
                *child = &nodes[nodes_used++];
            }
            current_node = *child;
        }

        if (current_node->is_leaf || current_node->left || current_node->right) {
            GFX_LOGE(TAG, "Invalid duplicate Huffman code");
            goto cleanup;
        }
        current_node->is_leaf = 1;
        current_node->symbol = symbol;
        if (++symbol_count > EAF_HUFFMAN_MAX_SYMBOLS) {
            GFX_LOGE(TAG, "Huffman dictionary exceeds symbol limit");
            goto cleanup;
        }
    }

    if (symbol_count == 0) {
        GFX_LOGE(TAG, "Empty Huffman dictionary");
        goto cleanup;
    }
    ...
    for (size_t bit_index = 0; bit_index < total_bits; bit_index++) {
        ...
        if (current_node->is_leaf) {
            if (out_pos >= out_capacity) {
                GFX_LOGE(TAG, "Huffman output overflow: %zu >= %zu", out_pos, out_capacity);
                goto cleanup;
            }
    ...
    if (current_node != root) {
        GFX_LOGE(TAG, "Incomplete Huffman code at end of block");
        goto cleanup;
    }
