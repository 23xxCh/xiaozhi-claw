...
static void emote_gen_player_free_preloaded_assets(emote_gen_player_handle_t handle)
{
    if (handle == NULL) {
        return;
    }

    if (handle->preloaded_assets != NULL) {
        for (size_t i = 0; i < handle->entry_capacity; i++) {
            if (handle->preloaded_assets[i] != NULL) {
                heap_caps_free(handle->preloaded_assets[i]);
            }
        }
        free(handle->preloaded_assets);
        handle->preloaded_assets = NULL;
    }
    free(handle->preloaded_asset_sizes);
    handle->preloaded_asset_sizes = NULL;
}

static esp_err_t emote_gen_player_preload_assets_to_spiram(emote_gen_player_handle_t handle)
{
    esp_err_t ret = ESP_OK;

    ESP_RETURN_ON_FALSE(handle != NULL && handle->mmap != NULL, ESP_ERR_INVALID_ARG, TAG, "invalid player");

    handle->preloaded_assets = calloc(handle->entry_capacity, sizeof(*handle->preloaded_assets));
    handle->preloaded_asset_sizes = calloc(handle->entry_capacity, sizeof(*handle->preloaded_asset_sizes));
    ESP_GOTO_ON_FALSE(handle->preloaded_assets != NULL && handle->preloaded_asset_sizes != NULL,
                      ESP_ERR_NO_MEM, fail, TAG, "alloc preload tables failed");

    size_t total_size = 0;
    for (size_t i = 0; i < handle->entry_count; i++) {
        const int asset_id = handle->entries[i].asset_id;
        ESP_GOTO_ON_FALSE(asset_id >= 0, ESP_ERR_NOT_FOUND, fail, TAG,
                          "animation asset not found: %s", handle->entries[i].file);

        const uint8_t *source = mmap_assets_get_mem(handle->mmap, asset_id);
        const size_t size = (size_t)mmap_assets_get_size(handle->mmap, asset_id);
        ESP_GOTO_ON_FALSE(source != NULL && size > 0, ESP_ERR_INVALID_SIZE, fail, TAG,
                          "invalid animation asset: %s", handle->entries[i].file);

        uint8_t *copy = heap_caps_malloc(size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        ESP_GOTO_ON_FALSE(copy != NULL, ESP_ERR_NO_MEM, fail, TAG,
                          "PSRAM allocation failed for %s (%zu bytes)", handle->entries[i].file, size);
        memcpy(copy, source, size);
        handle->preloaded_assets[i] = copy;
        handle->preloaded_asset_sizes[i] = size;
        total_size += size;
    }

    ESP_LOGI(TAG, "Preloaded %zu animations (%zu bytes) to PSRAM", handle->entry_count, total_size);
    return ESP_OK;

fail:
    emote_gen_player_free_preloaded_assets(handle);
    return ret;
}
...
esp_err_t emote_gen_player_mount_assets(emote_gen_player_handle_t handle, const emote_gen_player_data_t *data)
{
    ...
    if (data->flags.preload_to_spiram) {
        ret = emote_gen_player_preload_assets_to_spiram(handle);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "preload assets to PSRAM failed: %s", esp_err_to_name(ret));
            emote_gen_player_unmount_assets(handle);
            return ret;
        }
        /* Keep the mapping handle alive until normal player teardown. Unmapping
         * while the other core is executing an ESP-SR model can invalidate the
         * shared flash cache and raise an MMU entry fault. Animation decoding
         * still reads only the PSRAM copies after this point. */
        ESP_LOGI(TAG, "Animation decoding switched to PSRAM; flash mmap retained until teardown");
    }
