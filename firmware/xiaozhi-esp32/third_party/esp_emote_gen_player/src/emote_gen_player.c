...
static esp_err_t emote_gen_player_apply_anim_by_index(emote_gen_player_handle_t handle, size_t index, bool repeat)
{
    ...
    if (handle->preloaded_assets != NULL && handle->preloaded_asset_sizes != NULL) {
        anim_data = handle->preloaded_assets[index];
        anim_size = handle->preloaded_asset_sizes[index];
    } else {
        ESP_RETURN_ON_FALSE(handle->mmap != NULL, ESP_ERR_INVALID_STATE, TAG, "asset source is unavailable");
