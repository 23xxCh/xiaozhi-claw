...
class PreviewTargetTests(unittest.TestCase):
    def test_windows_runs_idf_py_with_the_active_python(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            idf_script = Path(temp_dir) / "tools" / "idf.py"
        ...
        self.assertEqual(command, [build.sys.executable, str(idf_script)])
