...
export type RouteCapabilities = { llm_temperature: boolean; tts_speech_rate: boolean; tools: boolean; supported_tool_ids: string[] };
export type ModelPreset = { id: string; display_name: string; description: string; is_default: boolean; route_kind: "cascade" | "realtime_s2s"; capabilities: RouteCapabilities; compatible_voice_ids: string[]; default_voice_preset_id: string | null };
...
export type Usage = { voice_turns: number; provider_cost_micros: number; pricing_configured: boolean; asr_units: number; llm_input_units: number; llm_output_units: number; tts_units: number; realtime_s2s_requests: number; unknown_cost_records: number };
