  ...
  return (
    <>
      ...
      <div className="agent-workspace">
        <section className="stack agent-list"><h2>我的助手</h2><p className="hint">每位助手拥有独立的声音和角色设定。</p>{agents.map((agent) => <button className={`button ${agent.id === selectedId ? "" : "secondary"}`} type="button" key={agent.id} disabled={submitting} aria-pressed={agent.id === selectedId} onClick={() => { chooseFrom(agents, agent.id); setMessage(null); }}>{agent.name}<span className="hint">{agent.device_count} 台设备</span></button>)}</section>
        <form className="card stack agent-editor" onSubmit={save}>
          <h2>定义你的助手</h2><p className="hint">保存后下一轮生效，不会打断正在播放的回复。</p>
          ...
          <p className="hint">试听展示基础音色；实际语气由语音方案决定。选择“应用默认音色”时沿用阿里配置，其他音色从下一轮生效。</p>
          ...
          </fieldset><fieldset className="editor-section"><legend>02 / 记忆</legend><label className="check"><input type="checkbox" checked={form.memory_consent} onChange={(event) => setForm({ ...form, memory_consent: event.target.checked })} />允许保存可查看、可删除的加密摘要记忆</label>
          <p className="hint">{selectedModel?.route_kind === "managed_app" && !selectedModel.capabilities.system_prompt ? "当前阿里应用尚未开放网页角色映射：可以保存摘要，但暂不用于此路线回答。" : "开启后，授权记忆会作为背景发送给所选语音服务；关闭会删除已保存的记忆。"} 摘要服务可能与语音服务不同。</p>
          </fieldset><details className="details"><summary>高级设置</summary><div className="stack">
          ...
          <div className="editor-footer"><span className="hint">仅应用于当前助手</span><button className="button" type="submit" disabled={!selectedId || !selectedModel || !selectedVoice || submitting}>{submitting ? "正在保存…" : "保存设置"}</button></div>
        </form>
      </div>
    </>
  );
