...
export default function AccountPage() {
  ...
  return <><header className="page-head"><div><div className="eyebrow">Account</div><h1>账户</h1><p>管理登录、显示偏好和公开版本说明。</p></div></header><ErrorMessage message={error} /><div className="grid two" style={{ marginTop: error ? 18 : 0 }}><section className="card stack"><h2>{me.display_name}</h2>{me.email ? <div>{me.email}</div> : <div className="warning">此历史账户尚未绑定邮箱，请退出后使用邮箱验证码完成本地迁移。</div>}<div className="hint mono">账户 {me.id.slice(0, 8)}…</div><span className={`status ${me.agreements_complete ? "online" : "warning"}`}>{me.agreements_complete ? "协议已确认" : "待确认协议"}</span><button className="button danger" type="button" disabled={loggingOut} onClick={() => void logout()}>{loggingOut ? "正在退出…" : "退出登录"}</button></section><section className="card stack"><h2>外观</h2><p className="muted">首次跟随系统，手动选择后会记住。</p><ThemeToggle /></section><section className="card soft" style={{ gridColumn: "1 / -1" }}><h2>当前公开范围</h2><p className="muted">当前版本仅供 18 岁以上成年人使用。家庭模式仍在内部验证，不代表已经向青少年开放。</p></section></div></>;
}
