...
将用户提供的九个“标准版表情包”GIF 接入 `hensun-nocam-pilot-v1`，替换当前过小的字体 Emoji，同时保留顶部设备状态和底部单行回复字幕。

## 接口设计

- 输入：固件现有 `Display::SetEmotion(const char*)` 情绪名称。
- 输出：与情绪匹配的 240×180 循环 GIF；未知情绪回退到 `neutral`。
...
- non-CAM 显示类只负责情绪别名映射，沿用通用 `SpiLcdDisplay` 的顶部状态栏和底部字幕栏。
- 构建、测试阶段不刷机；刷机前重新确认端口与授权，只允许更新应用及 `assets`，不得覆盖 NVS、Wi-Fi 或 `hensun_keys`。

## 验收

- 九个资源均为有效 240×180 动画 GIF，名称和映射完整。
