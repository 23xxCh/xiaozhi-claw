...
截至 2026-08-28，本地金样机已经验证：

- `3000` 前端、`8000` 控制面、`8001` 实时网关可同时运行。
- ESP32 可以连接本机 Bootstrap/WSS，完成唤醒、ASR、LLM、TTS、播放排空和 10 秒续聊。
...
- 回复完成后先显示约 0.8 秒收尾并经过扬声器余响保护，再从麦克风实际采集开始计算 10 秒续聊；无人讲话后静默进入睡眠表情。
- 语音、固件、构建/CI、设备配置迁移和文档已经按职责拆成本地提交；各提交均通过针对性测试。
- 后端为 133 passed、1 skipped，固件主机/Profile/资源测试为 121 passed，前端 ESLint、TypeScript 与 17 页面生产构建通过。
- ESP-IDF 6.0.2 的本地横屏版、官网回退版和通用自有版均已完整构建。
...
- 稳定性实现、设备配置迁移和交接文档已经分组提交；仅 `design/` 是明确排除的其他视觉工作线。不要 `git reset --hard`、`git checkout -- .`、清理未跟踪文件或整批 `git add -A`。
